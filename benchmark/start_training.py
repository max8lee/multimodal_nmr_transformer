...
def main(template_path: str, output_path: str, seed: int, train_from: str):
    ...
    cmd = ['onmt_train', '-config', input_file_path]
    
    if args.train_from:
        checkpoint_path = args.train_from
        cmd.extend(['-train_from', checkpoint_path, '-reset_optim', 'all'])
...
parser.add_argument('--template_path', default='./benchmark/base_transformer_template.yaml', help='Config path')
...
if __name__ == '__main__':
    main(args.template_path, args.output_path, args.seed, args.train_from)